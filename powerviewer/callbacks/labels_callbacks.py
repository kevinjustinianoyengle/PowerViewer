"""Label editing: title / axis titles / legend-box position + series renames.

Overrides live in ``labels-store`` keyed by viewer:
``{viewer: {"title", "xaxis", "yaxis", "yaxis2", "legend_pos",
            "series": {column: name}}}``.
"""
...
def register(app: Dash) -> None:
    ...
    def populate(active, labels):
        ...
        return (lab.get("title", ""), lab.get("xaxis", ""), lab.get("yaxis", ""),
                lab.get("yaxis2", ""), lab.get("legend_pos", "tr"))

    # --- Save title / axis / legend-position edits back into the store ------ #
    @app.callback(
        ...
        Input("label-legend-pos", "value"),
        State("active-view", "data"),
        State("labels-store", "data"),
        prevent_initial_call=True,
    )
    def save_labels(title, xaxis, yaxis, yaxis2, legend_pos, active, labels):
        ...
        entry["legend_pos"] = legend_pos or "tr"
        entry.setdefault("series", {})
        labels[active] = entry
        return labels

    # --- Save legend (series) renames (chip "Rename" inputs share these ids) - #
    @app.callback(
        Output("labels-store", "data", allow_duplicate=True),
